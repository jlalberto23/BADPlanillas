...
type Form = {
  id_deptoEmpresa: CentroCostoTable['id_deptoEmpresa']
  id_anio: CentroCostoTable['id_anio']
  presupuesto_total: CentroCostoTable['presupuesto_total']
}

export function CentroCostoEditingDialog({ children, centroCosto }: Props) {
  const [open, setOpen] = useState(false)
  const { data, setData, reset, put, processing, errors, clearErrors } = useForm<Form>({
    id_deptoEmpresa: centroCosto.id_deptoEmpresa,
    id_anio: centroCosto.id_anio,
    presupuesto_total: centroCosto.presupuesto_total
  })

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()

    put(route('payroll.centroscosto.update', { id: centroCosto.id_centro_costo }), {
      preserveScroll: true,
      onSuccess: () => {
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <form onSubmit={handleSubmit} className="grid gap-4" autoComplete="off">
          <AlertDialogHeader>
            <AlertDialogTitle>Editar Centro de Costo</AlertDialogTitle>
            <AlertDialogDescription>Modifica la información del centro de costo.</AlertDialogDescription>
          </AlertDialogHeader>
